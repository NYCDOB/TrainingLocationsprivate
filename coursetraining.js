...
d3.csv("data/CourseTrainingLocations.csv", function(data) {
	if (!data) {  //when there is a problem with the returned data
		...
		let justKeys = data.map( (d) => {  //  create a key, and since we're processing each line of data, also create latLngs & points arrays
		...
		$("#locationTable").on("click", "tr[level='1'] span:nth-child(3)", function(event){
			...
			if (! vParentRow.classList.contains("zoomed") ) {  //if the point is not zoomed, need to zoom to it
				( document.querySelector(".zoomed") ) ?  document.querySelector(".zoomed").classList.remove("zoomed") : 0;  //can only have one zoomed class at a time.
				vParentRow.classList.add("zoomed")   //will color the row background grey
				let vLocationID =  vParentRow.getAttribute("id")
				let vTargetLatLng=null;
				for ( let ctr=0;ctr<data.length;ctr++) {   //find this locations's matching row in data[], and grab its latLng
						//if (data[ctr].TrainingLocationID==vLocationID) {
						if (     data[ctr].TrainingLocationID==       vLocationID.substring(    vLocationID.indexOf('__')+2      )     ) {
							//substring(  vCoursesTable.rows[i].getAttribute("id").indexOf('__')+2)  )
				...
				document.querySelectorAll(".notMatched").forEach( (item) => item.classList.remove("notMatched")  );
							...
							if ( allCoursesFiltered )  {
								...
								if ( data.length != selectedCourse_cpID.length) { 
											...
											document.querySelector(".textSearchResults").style.display="inline" ;
								}			
								zoom(latLngs);
								let vBoros=new Set();
								...
								for (let i=0, totRows=vCoursesTable.rows.length; i < totRows; i++ ) {
										...
										} else if ( vCoursesTable.rows[i].getAttribute("level")=="1" &&  !vLocations.has(vCoursesTable.rows[i].getAttribute("id").substring(  vCoursesTable.rows[i].getAttribute("id").indexOf('__')+2)  ) ) {
											vCoursesTable.rows[i].classList.add("notMatched")
										} else  if ( vCoursesTable.rows[i].getAttribute("level")=="2" ) {
															if ( !vCourses.has(  vCoursesTable.rows[i].getAttribute("id").substring(  vCoursesTable.rows[i].getAttribute("id").lastIndexOf('__')+2) )  ) {
																	vCoursesTable.rows[i].classList.add("notMatched")		
															} else {
																	theparent=vCoursesTable.rows[i].getAttribute("id")
															}
										} else if ( vCoursesTable.rows[i].getAttribute("level")=="3" && !fullkey.has(vCoursesTable.rows[i].getAttribute("row-id")) )  {
														if ( vCoursesTable.rows[i].classList.contains("providerInfo")   )  {
															
														} else {
															vCoursesTable.rows[i].classList.add("notMatched")  //does not have this key
														}
										}
									}
	...
	function zoom(latLngs){
		if (latLngs === undefined || latLngs == 0){  //no courses match
			...
			$(".notFoundRow").css("display","none") ;			//clear the "not found" div	
	...
	function setLink() {   //window resize handling; do some things, such as changing hyperlinks
	  return;
	...
	setLink();   //initial setup upon document load; do things based on initial size of window
	...
	*/
	...
	function buildTblTreeElements(data, keyArray, languages) {			
			...
			let buildRow = function (  rowIDCol, theID=rowIDCol, tdString, rowParent='', linkable=false ) {
					theID = theID.replace(/\s+/g, '');					
					let parentString = rowParent ? ' parent-id="' + rowParent  +  '"'   :     '' ;
					 let linkString = linkable ? '<a href="#">' + tdString + '</a>' : tdString;
					// let linkString = linkable ? tdString +  '&ensp;<a title="See Approved Courses" target="_blank " href="https://www1.nyc.gov/site/buildings/industry/dob-approved-courses.page"><span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span></a>' : tdString;
					let mainString = '<tr row-id="' + rowIDCol + '"' + parentString +" id='" + theID +"'><td><b>" +	linkString + '</b></td></tr>';
					return mainString;
				};
			let buildRow2 = function (  rowIDCol, theID=rowIDCol,  tdString, rowParent='', linkable=false ) {
				theID = theID.replace(/\s+/g, '');
				let parentString = rowParent ? ' parent-id="' + rowParent  +  '"'   :     '' ;
				let linkString = (linkable) ? //build proper link when protocol is missing
				   '<a target="_blank" href="'  + ((tdString[1].indexOf('//') == -1)  ? "http://" :"")+
					tdString[1] + '">' + tdString[1] + '</a>':
					tdString[1]; //just display the column's value
				let mainString = '<tr row-id="' + rowIDCol + '"' + parentString +" id='" + theID + "'><td><span class='detailHead' > " + tdString[0] + '</span>' +  "<span>"  +  linkString +  "<span>"   + "</td></tr>" ;  //FROM ORIGINAL
				return mainString;
			};			
			...
			let alldata2 = keyArray.map(function(q, indexNum, array) {  //q=current element in array of keys
				...
				if ( !boroughsset.has(d.Borough) ) {  //add a new borough heading,  level 0  BOROUGH
						...
						let newText = document.createTextNode(d.Borough);
						newCell.appendChild(newB);
						newB.appendChild(newText);
						newRow.setAttribute("row-id",d.Borough);
						newRow.id=d.Borough;
				} //end boro				
				if ( !boroughlocationset.has(d.Borough+"__"+d.TrainingLocationID)    ) {  //level 1 STREET ADDRESS
					...
					let newRow =tableRef.insertRow(-1);
					let newCell = newRow.insertCell(0);
					let newText = document.createTextNode(d.Street +( (d.newBoro == 6 ) ? " ("+d.City+ ", " + d.State +")": "" ));
					let newSpan=document.createElement("span");
					newCell.appendChild(newSpan);
					newSpan.appendChild(newText);
					newRow.setAttribute("row-id",d.Borough+"__"+d.TrainingLocationID);
					newRow.setAttribute("parent-id",d.Borough);
				...
				if ( !boroughlocationcourseset.has(d.Borough+"__"+d.TrainingLocationID+"__"+d.CourseID)) {  //level 2  COURSE NAME
								...
								boroughlocationcourseset.add(myID);	
								myParent = d.Borough+"__"+d.TrainingLocationID;
								firstlocationcourse=true; // ************************************
								boroughlocationcourseset.add(d.Borough+"__"+d.TrainingLocationID+"__"+d.CourseID);	
								let newRow =tableRef.insertRow(-1);
								let newCell = newRow.insertCell(0);
								let newText = document.createTextNode(d.Course);
								let newSpan=document.createElement("span");
								
								newCell.appendChild(newSpan);
								newSpan.appendChild(newText);
								
								newRow.setAttribute("row-id",d.Borough+"__"+d.TrainingLocationID+"__"+d.CourseID);
								newRow.setAttribute("parent-id",d.Borough+"__"+d.TrainingLocationID);
				...
				if ( !boroughlocationcoursedatetimestartentryset.has(q )){  //level 3  COURSE DATE/TIME
							myID=q;
							boroughlocationcoursedatetimestartentryset.add(q);   //myID);
							...
							let newText2 = document.createTextNode(d["Start Date"]+" " + d["Start Time"] + ((d["End Time"] ) ? " - "+d["End Time"]:" ")+  //end time is sometimes missing
									" (" + ((d.Instructor)? d.Instructor:"(tbd)") + ")");
							...
							if  ( indexNum == array.length-1 || (q.substr(0,9) !=  array[indexNum+1].substr(0,9))  ) {
								...
								newCell.appendChild(newSpan1);
								newCell.appendChild(newSpan2);
								newRow.setAttribute("row-id",q+"__provider");
								newRow.setAttribute("parent-id",d.Borough+"__"+d.TrainingLocationID+"__"+d.CourseID);
								...
								newA.target="_blank";
								newA.href=d["CourseProviderWebsite"];
								newA.appendChild(newText2);
								...
								newSpan1 = document.createElement("span");
								newCell.appendChild(newSpan1);
								newRow.setAttribute("row-id",q+"__separator");
								newRow.setAttribute("parent-id",d.Borough+"__"+d.TrainingLocationID+"__"+d.CourseID);
								newRow.id=q+"__separator";
							}							
				} 
			}) //END alldata2 var
